...
TILENAME_SEED = 42
tiles = pd.read_csv(os.environ['RUN_DIR'] + '/data/Tilelist_DR3_2.csv')
...
if __name__ == '__main__':
    ...
    tilenames = tilenames[0:args['TileCount']]
    print(tilenames)
    ...
    def prep_job(tilename):
        ...
        commands = []
        for b in 'riz':
            commands.append('python $RUN_DIR/run_sims.py prep --tilename="%s" --bands="%s" --output-desdata="$PREP_DIR/%s/outputs_%s_gplus" --config-file="config_plus.yaml"'%(tilename, b, name, tilename))
    ...
    #In next step we keep track of all jobs and add to queue when we can
        ...
        while (dt.datetime.now() - start).seconds < 3600*48: #48 hours max
            ...
            if current_job_count() >= args['MaxConcurrentJobs']:
                ...
                #Try clause needed a some tiles expected to fail DB query
                #since monica is deleting some tiles now.
                try: 
                    prep_job(t)
                    ...
                    print("FAILED JOB %s. DONT THINK QUERY WORKED" % j)
